...
function Swap() {
  ...
  const etherChange = async (event) => {
    ...
    if(Number(event.target.value) > 0.000000000000000001 ){
      ...
      const pair = await Fetcher.fetchPairData(uniswapContract, WETH[uniswapContract.chainId]);
  ...
  const swap = async (event) => {
    ...
    const gasEstimate = await RouterContract.getInstance().estimate.swapExactETHForTokens(
      amountOutMin,
      path,
      to,
      deadline,
  ...
  return ( 
    <div>
        ...
        <form onSubmit={ swap }>
          ...
          <br/>
          <button disabled={!isConnected} type="submit">Swap Token</button>
        </form>
    </div>
  );
}
